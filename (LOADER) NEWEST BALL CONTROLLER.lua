--========================================================--
-- BALL CONTROLLER V4.5 (WITH STANDALONE MORE MODS LOADER)
...
do
    ...
    -- Floating mobile/desktop action controls
...
do
    ...
    function API.applyResponsiveUI()
        ...
        if narrow then
            ...
            UI.tpTimeLabel.Position = UDim2.fromOffset(15, 310)
            UI.tpTimeLabel.Size = UDim2.fromOffset(80, 38)
            UI.tpTime.Position = UDim2.fromOffset(100, 310)
            UI.tpTime.Size = UDim2.fromOffset(colW - 85, 38)
            UI.moreMod.Size = UDim2.fromOffset(colW, 38)
            UI.moreMod.Position = UDim2.fromOffset(15 + colW + gap, 310)
            ...
            UI.tpTimeLabel.Position = UDim2.fromOffset(10,310)
            UI.tpTimeLabel.Size = UDim2.fromOffset(85,36)
            UI.tpTime.Position = UDim2.fromOffset(100,310)
            UI.tpTime.Size = UDim2.fromOffset(70,36)
            UI.moreMod.Position = UDim2.fromOffset(180,310)
            ...
            UI.m2.speedLabel.Size=UDim2.fromOffset(10,10)
...
do
    ...
    -- LOADSTRING LOADER CHO MORE MODS
    API.bind(UI.moreMod.MouseButton1Click, function()
        task.spawn(function()
            local ok, err = pcall(function()
                loadstring(game:HttpGet("https://raw.githubusercontent.com/thatonevietnamese/BLUE-LOCK-SKIBIDI-YESSIR/refs/heads/main/(LOADER)standard.lua"))()
            end)
            if not ok then
                API.notify("MORE MODS", "Execution failed: " .. tostring(err), 2.0)
            else
                API.notify("MORE MODS", "Loaded Standard Loader", 1.2)
            end
        end)
    end)
...
API.notify("BALL CONTROLLER", "V4.5 Loaded with More Mods Loader", 1.4)

print("[Ball Controller V4.5 Loader Version] loaded")
