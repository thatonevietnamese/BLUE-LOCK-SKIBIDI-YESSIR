...
local State = {
	...
	specialStealActive = false,
	specialStealStep = 1,
	specialStealTarget = nil,
	specialStealNextAt = 0,
	specialStealLastActionAt = 0,
...
local title = makeLabel(main, "⚽ BALL CONTROLLER V4.3", 10, 5, 215, 35, 18)
title.Font = Enum.Font.GothamBold
...
local CharacterAbilities = {
	default = {
		canSlideSteal = true,
		-- Only Barou can slide-steal a teammate.
		canSlideStealTeammate = false,

		-- nil = no automatic special steal.
		specialSteal = nil,
	},

	barou = {
		-- All characters can slide-steal opponents. Barou alone can
		-- use the normal slide to steal from a teammate.
		canSlideSteal = true,
		canSlideStealTeammate = true,

		-- Skill 3: TP above the opponent and repeatedly press 3 while
		-- the opponent still owns the ball.
		specialSteal = {
		name = "BAROU 3",
		...
		allowNormalSlideFallback = true,
		interval = 0.14,
		tpOffset = Vector3.new(0, 5.5, 0),
	},
},
	...
	naoya = {
		...
		-- 2 steals first. Only after local possession is detected,
		-- 3 is fired as the follow-up.
		specialSteal = {
			name = "NAOYA 2 → 3",
			targetMode = "OPPONENT",
			method = "TPBFUSE",
	...
	kaiser = {
		-- Kaiser can still use the normal slide against opponents, but
		-- the slide may NEVER steal a teammate. Skill 3 is the only
		-- Kaiser action allowed to steal a teammate.
		canSlideSteal = true,
		canSlideStealTeammate = false,
	...
	gagamaru = {
		...
		-- This skill knocks the ball away instead of making us the holder.
		specialSteal = {
			name = "GAGAMARU 3",
			key = Enum.KeyCode.Three,
			targetMode = "OPPONENT",
	...
	-- Alias for servers/versions that expose the name with the common typo.
	chirigi = {
		canSlideSteal = true,
		canSlideStealTeammate = false,
		specialSteal = {
...
local function getCharacterAbility()
	local name = string.lower(string.gsub(getCharacterNameValue(), "%s+", ""))
	return CharacterAbilities[name] or CharacterAbilities.default
end

local function localCharacterName()
	return string.lower(string.gsub(getCharacterNameValue(), "%s+", ""))
end

local function localIsBarou()
	return localCharacterName() == "barou"
end

local function resetSpecialStealState()
	State.specialStealActive = false
	State.specialStealStep = 1
	State.specialStealTarget = nil
	State.specialStealNextAt = 0
	State.specialStealLastActionAt = 0
end
...
local function findBallTarget()
	local ability = getCharacterAbility()

	if not ability.canSlideSteal then
		return nil, nil
	end
	...
	if state == "HELD" and holder then
		...
		local same = sameTeam(holder)

		if same and not ability.canStealTeammate then
			return nil, nil
		end
...
local function getSpecialStealTarget(special)
	local state, holder, ball = getBallState()

	if state ~= "HELD" or not holder or holder == player then
		return nil, nil
	end

	local mode = special.targetMode or "OPPONENT"

	if mode == "OPPONENT" and sameTeam(holder) then
		return nil, nil
	end

	if mode == "TEAMMATE" and not sameTeam(holder) then
		return nil, nil
	end

	if mode == "ANY" and sameTeam(holder) and not getCharacterAbility().canStealTeammate then
		return nil, nil
	end
...
local function tpbfuse(targetRoot, keyCode, offset)
	if not targetRoot or not targetRoot.Parent then
		return false
	end
	...
	local currentRoot = rootPart
	local tpOffset = offset or Vector3.new(0, 5.5, 0)

	currentRoot.CFrame = CFrame.new(
		targetRoot.Position + tpOffset,
		targetRoot.Position
	)

	return sendVirtualKey(keyCode)
end

local function specialStealStep()
	local ability = getCharacterAbility()
	local special = ability.specialSteal

	if not special or not special.key and not special.steps then
		return false
	end

	local now = os.clock()

	if not State.specialStealActive then
		local targetRoot, targetPlayer = getSpecialStealTarget(special)

		if not targetRoot then
			return false
		end

		State.specialStealActive = true
		State.specialStealStep = 1
		State.specialStealTarget = targetPlayer
		State.specialStealNextAt = 0
		State.specialStealLastActionAt = 0
	end

	local targetPlayer = State.specialStealTarget
	...
	then
		resetSpecialStealState()
		return false
	end

	-- The target must still own the ball before a steal action is repeated.
	local ballState, ballHolder = getBallState()

	if ballState ~= "HELD" or ballHolder ~= targetPlayer then
		if special.steps
			and State.specialStealStep <= #special.steps
			and special.steps[State.specialStealStep].waitForPossession
			and localHasBall()
		then
			-- Continue the combo below.
		else
			resetSpecialStealState()
			return false
		end
	...
	if special.steps then
		step = special.steps[State.specialStealStep]
		...
		if step.waitForPossession and not localHasBall() then
			return true
		end
	else
		step = special
	...
	if step.waitForPossession and localHasBall() and State.specialStealStep < #special.steps then
		State.specialStealStep += 1
		step = special.steps[State.specialStealStep]
	end
	...
	local interval = special.interval or 0.16

	if now < State.specialStealNextAt then
		return true
	end

	local keyCode = step.key or special.key
	...
	if (step.method or special.method) == "TPBFUSE" then
		used = tpbfuse(targetRoot, keyCode, step.tpOffset or special.tpOffset)
	else
		used = sendVirtualKey(keyCode)
	end
	...
	State.specialStealLastActionAt = now
	State.specialStealNextAt = now + interval

	if step.finishAfterUse or special.oneShot then
		resetSpecialStealState()
	end
...
local function spamStealStep()
	...
	-- Every character can use the normal slide against opponents.
	-- A special skill gets priority for the current tick; if it cannot act,
	-- the normal opponent slide remains available.
	local usedSpecial = specialStealStep()
	if usedSpecial then
		return
	end
...
local function tryPhysicsAutoGoal(ball, goalHitbox)
	if not ball or not ball.Parent or not ball:IsA("BasePart") then
		return false
	end

	if ball.Anchored or not goalHitbox or not goalHitbox.Parent then
		return false
	end

	local delta = goalHitbox.Position - ball.Position
	if delta.Magnitude < 0.001 then
		return false
	end

	local ok = pcall(function()
		-- FREE ball branch: when the client can control the ball physics,
		-- place the ball directly on the opponent goal hitbox.
		ball.CFrame = goalHitbox.CFrame
		ball.AssemblyLinearVelocity = Vector3.zero
		ball.AssemblyAngularVelocity = Vector3.zero
	end)
	...
	-- This branch is only attempted for a FREE ball. pcall() proves the
	-- local write succeeded, not that the server will necessarily replicate it.
	return true
end
...
local function performAutoGoalStep()
	...
	-- 1) Local already owns the ball: TP the PLAYER, then shoot with ShootBall.
	if state == "HELD" and holder == player and ball and ball.Parent then
		if not updateCharacter() or not rootPart or not rootPart.Parent then
			return false
		end
		...
		task.spawn(function()
			...
			local ok, err = pcall(function()
				...
				if direction.Magnitude > 0.001 then
					if not fireShootRemote(direction.Unit, SHOOT_FORCE, false) then
						notify("AUTO GOAL", "ShootBall không khả dụng", 1.2)
					end
				end
			end)
			...
			-- Restore only the PLAYER. The ball is never teleported by Auto Goal.
	...
	-- 2) Local does not own the ball. A FREE ball can be directly controlled
	-- by the client and placed on the opponent goal hitbox.
	-- If an opponent owns it, the Heartbeat steal path gets possession first.
	if state == "FREE" and ball and ball.Parent then
		return tryPhysicsAutoGoal(ball, goalHitbox)
	end
...
Bind(RunService.Heartbeat, function()
	...
	if State.autoGoalEnabled then
		local autoGoalState, autoGoalHolder = getBallState()

		if autoGoalState == "HELD" and autoGoalHolder == player then
			resetSpecialStealState()
			performAutoGoalStep()
		elseif autoGoalState == "FREE" then
			resetSpecialStealState()
			performAutoGoalStep()
		else
			-- Opponent-owned ball: steal first. Once local possession is detected,
			-- the next Heartbeat enters the ShootBall branch above.
			spamStealStep()
		end
...
notify("BALL CONTROLLER", "V4.3 loaded", 1.5)

print("[Ball Controller V4.3] loaded")
